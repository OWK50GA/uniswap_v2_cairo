...
pub mod Pair {
    ...
    pub impl PairImpl of IPair<ContractState> {
        fn mint(ref self: ContractState, to: ContractAddress) {
            ...
            // let lptoken_dispatcher = ILPTokenDispatcher { contract_address: self.lp_token.read()
            // };
            // let total_shares = lptoken_dispatcher.get_total_supply();
        ...
        fn burn(ref self: ContractState, to: ContractAddress) {
            let (reserve0, reserve1, _) = self.get_reserves();
            ...
            // let new_lptoken_dispatcher = ILPTokenDispatcher { contract_address:
            // self.lp_token.read() };
    ...
    pub impl InternalFunctions of InternalTrait {
        ...
        fn _mint_fee(ref self: ContractState, reserve0: u256, reserve1: u256) -> bool {
            let factory_dispatcher = IFactoryDispatcher { contract_address: self.owner.read() };
            // let lptoken_dispatcher = ILPTokenDispatcher { contract_address: self.lp_token.read()
            // };
            // let erc20_dispatcher = IERC20Dispatcher { contract_address: self.lp_token.read() };
