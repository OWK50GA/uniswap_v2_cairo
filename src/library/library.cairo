pub mod Library {
    ...
    // calculates the address for a pair without making any external calls (thanks to deterministic values)
    pub fn pair_for(
        // self: @ComponentState<TContractState>,
    ...
    pub fn get_amounts_out(
        factory: ContractAddress, amount_in: u256, path: Array<ContractAddress>, pair_class_hash: ClassHash
    ) -> Array<u256> {
        ...
        for i in 0..path.len() {
            let (reserve_in, reserve_out) = get_reserves(factory, pair_class_hash, *path.get(i).unwrap().unbox(), *path.get(i).unwrap().unbox());
            amounts.append(get_amount_out(*amounts.get(i).unwrap().unbox(), reserve_in, reserve_out));
        }
    ...
    pub fn get_amounts_in(
        ...
        // for i in 0..path.len() {
        //     let (reserve_in, reserve_out) = self.get_reserves(factory, *path.at(path_length - i - 2), *path.at(path_length - i - 1));
        //     amounts.append(self.get_amount_in(*amounts.get(i - 1).unwrap().unbox(), reserve_in, reserve_out))
        // }
        ...
        while i < (path_length - 1) {
            ...
            let (reserve_in, reserve_out) = get_reserves(factory, pair_class_hash, *path.get(idx_in).unwrap().unbox(), *path.get(idx_out).unwrap().unbox());
